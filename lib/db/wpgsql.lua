...
function wpgsql:val_params(fields_num, col_num)
    ...
    for i=1, fields_num do
        if (limit_flags[i]) then
            ...
            if (i == fields_num) then break end
            val_str = val_str .. ', ('
        else
            val_str = val_str .. '$' .. tostring(i) .. ', '
        end
...
function wpgsql:list_obj(obj, col_name)
    if (not obj or not col_name) then return false end
    if (type(obj) ~= 'table' or type(col_name) ~= 'string') then return false end
    local list = ' ('
    local del = ', '
    for i=1, #obj do
        if (i == #obj) then del = ') ' end
        if obj[i][col_name] then
            list = list .. '\'' .. tostring(obj[i][col_name]) .. '\'' .. del
        else
            return false
        end
    end
...
function wpgsql:list_obj_params(obj, col_name)
    ...
    for i=1, #obj do
        if (i == #obj) then del = ')' end
        if obj[i][col_name] then
            list = list .. self:add_param(tonumber(obj[i][col_name])) .. del
        else
            return false
        end
    end
...
function wpgsql:to_obj()
    ...
    if (r == 0 or c == 0) then return nil, nil end
    local c_names, obj = {}, {}
    for i=1, c do
