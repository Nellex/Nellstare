...
--
-- 2018 Senin Stanislav, added:
-- _ip validator, with port number
...
--
function valua._ip(value, wport)
    --modes: wport (with port 1.2.3.4:8080)

    local p1, p2 = value:find('^[1-9][0-9]?[0-9]?%.[0-9][0-9]?[0-9]?%.[0-9][0-9]?[0-9]?%.[1-9][0-9]?[0-9]?')
    ...
    local oc1, oc2, oc3, oc4 = value:match('^([1-9][0-9]?[0-9]?)%.([0-9][0-9]?[0-9]?)%.([0-9][0-9]?[0-9]?)%.([1-9][0-9]?[0-9]?)')
    oc1 = tonumber(oc1)
    oc2 = tonumber(oc2)
    oc3 = tonumber(oc3)
    oc4 = tonumber(oc4)

    if (oc1 == 0 or oc1 > 254) then
        return false
    end

    if (oc2 > 255 or oc3 > 255 or oc4 > 255) then
        return false
    end

    if (not wport and p2 == #value) then
        return true
    end

    if (wport and (p2 + 1) < #value and value:sub(p2 + 1, p2 + 1) == ':') then
        local port = string.match(value:sub(p2 + 2, #value), '[1-9][0-9]*$')
