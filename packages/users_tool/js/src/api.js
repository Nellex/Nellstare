...
const setGroupPropertiesViewMessage = (message, severity) => {
    ...
    usersToolState.groupProperties.visible.get() == true ? usersToolState.groupProperties.hasMessage.set(true) :
    usersToolState.groupProperties.hasMessage.set(false);
}

const getGroups = async () => {
    let res = await authReq({meth: 'get_grps'});
...
const getUnits = async () => {
    let res = await authReq({meth: 'get_units'});
...
const prepareGroupPolicies = async() => {
    ...
    for (let i = 0; i < units.length; i++) {
        groupPolicies.push({id: units[i]['id'], unit_name: units[i]['unit_name'], r_p: false, w_p: false});
...
const getGroupPolicies = async(groupName) => {
    ...
    for (let i = 0; i < res.jsonData.length; i++) {
        ...
        groupPolicies.merge(() => ({
            [groupPoliciesMap[row.unit_id].value]: {id: row.unit_id, unit_name: unitName, r_p: row.r_p == 't' ? true : false, w_p: row.w_p == 't' ? true : false}
        }));
    }
...
const sendDeleteGroupReq = async(groupName) => {
    if (groupName == 'admin') { // нельзя удалять админскую группу
        return {raw: false}
    }
...
const getUsers = async () => {
    let res = await authReq({meth: 'get_usrs'});
...
export {
    ...
    setGroupsViewMessage,
    getGroupPropertiesViewMessage,
    setGroupPropertiesViewMessage,
    getGroups,
    getUnits,
    prepareGroupPolicies,
    getGroupPolicies,
    sendUpdatePolicyReq,
    sendAddPolicyReq,
    sendAddGroupReq,
