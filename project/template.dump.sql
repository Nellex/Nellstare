...
INSERT INTO config VALUES('project_priority','2');
...
CREATE TABLE packages (name VARCHAR (255) PRIMARY KEY UNIQUE NOT NULL, status REFERENCES package_status (status) ON DELETE CASCADE ON UPDATE CASCADE MATCH FULL);
CREATE TABLE autoload_modules (
    package                   REFERENCES packages (name) ON DELETE CASCADE
                                                         ON UPDATE CASCADE
                                                         MATCH [FULL]
                              NOT NULL,
    module_name VARCHAR (255) NOT NULL
);
CREATE UNIQUE INDEX pidx ON acl (source COLLATE BINARY COLLATE BINARY ASC, target COLLATE BINARY COLLATE BINARY ASC);
