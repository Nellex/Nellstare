    ...
    if (itemsMap[categories[i].id] && itemsMap[categories[i].id].length > 0) {
      for (let j = 0; j < itemsMap[categories[i].id].length; j++) {
        ...
        childs.push({
          ...
          icon: icon,
          command: () => {
            runApp(Number(unitId), title, reqName);
          }
        });
      }
    }
...
const addAppComponent = (id, appComponent, appState) => {
  ...
  hubState.appComponents.merge({
    [id]: appComponent
  });
  // add app layout
  hubState.appsLayout.merge({
    [id]: 'default'
  });
};
...
const renderActiveApp = (id, HlayoutTemplate, VlayoutTemplate) => {
  ...
  if (appLayout == 'default') {
    return /*#__PURE__*/React.createElement("div", null, React.createElement(hubState.appComponents[id].get(), {
      state: appsState.get(id)
    }));
  }
  if (appLayout == 'hLayout') {
    ...
    if (leftPosId in hubState.appComponents) {
      leftApp = () => React.createElement(hubState.appComponents[leftPosId].get(), {
        state: appsState.get(leftPosId)
      });
    }
    if (rightPosId in hubState.appComponents) {
      rightApp = () => React.createElement(hubState.appComponents[rightPosId].get(), {
        state: appsState.get(rightPosId)
      });
    }
    return /*#__PURE__*/React.createElement(HlayoutTemplate, {
      left: leftApp,
      right: rightApp,
      active: id,
      leftId: leftPosId,
  ...
  if (appLayout == 'vLayout') {
    ...
    if (topPosId in hubState.appComponents) {
      topApp = () => React.createElement(hubState.appComponents[topPosId].get(), {
        state: appsState.get(topPosId)
      });
    }
    if (bottomPosId in hubState.appComponents) {
      bottomApp = () => React.createElement(hubState.appComponents[bottomPosId].get(), {
        state: appsState.get(bottomPosId)
      });
    }
    return /*#__PURE__*/React.createElement(VlayoutTemplate, {
      top: topApp,
      bottom: bottomApp,
      active: id,
      topId: topPosId,
      bottomId: bottomPosId
    });
  }
  return null;
};
...
export { getSidebarMenuData, makeMenu, runApp, addActiveAppItem, delActiveAppItem, getAppName, closeApp, selectActiveAppItem, markSplittedActiveAppItem, unmarkSplittedActiveAppItem, addAppComponent, addToHlayout, addToVlayout, getLayoutPos, removeFromLayout, renderActiveApp, logout };
